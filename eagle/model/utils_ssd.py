...
def initialize_tree(input_ids, draft_input_ids, model, past_key_values, logits_processor, embed_model, pixel_values, image_sizes):
    ...
    batch_filtered_ids    = []
    batch_filtered_embeds = []
    device = input_ids.device

    for ids_seq, embeds_seq in zip(input_ids, input_embeds):
        img_count = 0
        keep_ids    = []
        keep_embeds = []
        # 遍历序列
        for tok_id, vec in zip(ids_seq.tolist(), embeds_seq):
            ...
            keep_embeds.append(vec)

        # 合成 Tensor 并收集
        batch_filtered_ids.append(torch.tensor(keep_ids, device=device))
        # 先 stack，再 .to(device)
        batch_filtered_embeds.append(torch.stack(keep_embeds, dim=0).to(device))

    # 3. （可选）如果后续需要固定长度，可以在这里做 pad：
    max_len = max(seq.size(0) for seq in batch_filtered_ids)
    B, D    = input_embeds.size(0), input_embeds.size(2)

    padded_ids    = input_ids.new_zeros((B, max_len))
    padded_embeds = input_embeds.new_zeros((B, max_len, D))

    for i, (ids_seq, embs_seq) in enumerate(zip(batch_filtered_ids, batch_filtered_embeds)):
        L_i = ids_seq.size(0)
        padded_ids[i, :L_i]       = ids_seq
        padded_embeds[i, :L_i, :] = embs_seq

    # 4. 最终输出
    filtered_input_ids    = padded_ids       # (B, max_len)
    filtered_input_embeds = padded_embeds    # (B, max_len, D)
    # Clone the output hidden states
    print(f"input ids shape {input_ids.shape}, {input_ids[0][:20]}, {input_ids[0][-20:]}, draft ids shape {draft_input_ids.shape}, {draft_input_ids}")
    draft_tokens, retrieve_indices,tree_mask,tree_position_ids = model.ea_layer.topK_genrate(hidden_states, filtered_input_ids, model.base_model.lm_head,logits_processor, filtered_input_embeds)
...
def update_inference_inputs(
    ...
    draft_input_ids = torch.cat(
        [draft_input_ids, candidates[None, best_candidate, : accept_length + 1].to(draft_input_ids.device)], dim=-1
    )
    ...
    draft_tokens, retrieve_indices,tree_mask,tree_position_ids = model.ea_layer.topK_genrate(accept_hidden_state_new,
                                              input_ids=torch.cat((draft_input_ids, token.to(draft_input_ids.device)), dim=1),
