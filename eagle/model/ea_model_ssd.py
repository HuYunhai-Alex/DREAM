...
class EaModel(nn.Module):
    ...
    def ea_generate(
        ...
        input_ids = inputs.input_ids.to(self.base_model.device)
        draft_input_ids = draft_inputs.input_ids.to(self.base_model.device)
        pixel_values = inputs.pixel_values.to(self.base_model.device).to(torch.bfloat16) if hasattr(inputs, "pixel_values") else None
        image_sizes = inputs.image_sizes.to(self.base_model.device) if hasattr(inputs, "image_sizes") else None
        ...
        start = time.time()
        draft_tokens, retrieve_indices,tree_mask,tree_position_ids, logits, hidden_state, sample_token = initialize_tree(
            input_ids, draft_input_ids, self, past_key_values, logits_processor, self.embed_model, pixel_values, image_sizes
        )
        t0 = time.time() - start
        t1 = t2 = 0
        ...
        for idx in range(max_length):
            ...
            if self.tokenizer.eos_token_id in input_ids[0, input_len:].tolist():
                print(f"len {len(input_ids[0, input_len:])}, init time {t0}, verify time {t1}, verify time {t1/idx}, draft time {t2}, draft time {t2/idx}")
                break
            if new_token > max_new_tokens:
                print(f"len {len(input_ids[0, input_len:])}, init time {t0}, verify time {t1}, verify time {t1/idx}, draft time {t2}, draft time {t2/idx}")
                break
